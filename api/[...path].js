...
async function ensureAdmin(){
  ...
  if(ai<0){
    ...
    // Keep the configured password in sync so an Admin can recover access
    // after a deployment without touching Redis manually.
    users[ai]={...users[ai],password_hash:bcrypt.hashSync(ADMIN_PASSWORD,10),profile:{...(users[ai].profile||{}),status:"approved"}};
    await put(K.users,users);
  }
  return true;
}
...
export default async function handler(req){
  try{
    ...
    try{ await seed(); }catch(e){ return fail("Database initialization failed: "+String(e?.message||e),503); }
    ...
    if(parts.length===1&&parts[0]==="health"&&method==="GET"){
      try{
        ...
        return json({ok:true,database:true,users:users.length,admin_configured:!!process.env.ADMIN_EMAIL,admin_email:ADMIN_EMAIL,kv_url_configured:!!url,kv_token_configured:!!token});
      }catch(e){
        return json({ok:false,database:false,error:String(e?.message||e),kv_url_configured:!!url,kv_token_configured:!!token},503);
      }
    }
    ...
    if(parts[0]==="profile-master"&&parts.length===1&&method==="GET"){
      return json((await redis.get(K.profileMaster))||{name:[],hq:[],abm:[],rm:[],region:["Nagpur"]});
    }
    if(parts[0]==="admin"&&parts[1]==="profile-master"&&method==="POST"){
      if(me.role!=="admin")return fail("Admin only",403);
      const m=(await redis.get(K.profileMaster))||{name:[],hq:[],abm:[],rm:[],region:["Nagpur"]};
      ...
      await redis.set(K.profileMaster,m);return json(m);
    }
    ...
    if(parts[0]==="skill"&&parts[1]==="award"&&method==="POST"){
      const xp=Math.max(0,Math.min(200,+b.xp||0));await updateUser(me.id,x=>({...x,xp:x.xp+xp,coins:x.coins+Math.floor(xp/10)}));return json({ok:true,xp});
    }
    if(parts[0]==="admin"&&parts[1]==="skill"&&method==="POST"){
      if(me.role!=="admin")return fail("Admin only",403);
      const a=await arr(K.skills),id=a.length?Math.max(...a.map(x=>x.id))+1:1,x={id,...b,created_at:Date.now()};a.push(x);await put(K.skills,a);return json(x);
    ...
    if(parts[0]==="admin"&&parts[1]==="hierarchy"&&method==="GET"){
      if(me.role!=="admin")return fail("Admin only",403);
